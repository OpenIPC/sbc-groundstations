...
if [[ -n $ETH_Fixed_ip && -n $ETH_Fixed_ip2 ]]; then
	...
	ETH_Fixed_ip_OS=$(nmcli -g ipv4.addresses  con show eth0)
	[ $ETH_Fixed_ip_OS == "${ETH_Fixed_ip}, ${ETH_Fixed_ip2}" ] || nmcli con modify eth0 ipv4.addresses ${ETH_Fixed_ip},${ETH_Fixed_ip2}
fi
